...
if (!is.na(file)){
  ...
  print("...from ARC")
  temp_file1 <- tempfile()
  ssh_command <- sprintf("ssh %s@%s '%s'", 'chaoe', 'arc.ohsu.edu', sacct_command)
  system(paste(ssh_command, ">", output_file), wait = TRUE)
  
  #print("...from Exacloud")
  #temp_file2 <- tempfile()
  #ssh_command <- sprintf("ssh %s@%s '%s'", 'chaoe', 'exahead1.ohsu.edu', sacct_command)
  #system(paste(ssh_command, ">", temp_file2), wait = TRUE)
  #system(paste("cat", temp_file1, ">", output_file, "&&", "tail -n +2", temp_file2, ">>", output_file))
  data <- fread(output_file)
...
if (length(unique(total.dt$User)) > 1) { 
  DT::datatable(total.dt %>% group_by(State, User) %>% summarise(NumJobs = n(), CPUCost = round(sum(CPUCost, na.rm = TRUE),2), GPUCost = round(sum(GPUCost, na.rm = TRUE), 2), AggCPUEff=round(mean(CPUEff, na.rm = TRUE), 2)), extensions =  c('Buttons'), options = list(initComplete = JS(
    "function(settings, json) {",
    "$(this.api().table().header()).css({'background-color': '#000', 'color': '#fff'});",
...
ellipsis_cols = c("JobNames", "SubmitLine")
...
```{r}
total.dt <- merge(total.dt, by_jobname[, c("SubmitLine", "SubmitShort")], by = "SubmitLine", all.x = TRUE)
...
NOTE: For MISC jobs, filter by req CPUs and order by ascending CPU Eff to see the ones that need improvement! 
  ...
  for(x in unique((by_jobname %>% filter(NumCompletedJobs > 5))$User)) {
    ...
    for(subtab in subtabs) {
      ...
      cat('\n')
        ...
        for (x in chunks){ 
         ...
         cat('<div class="superbigimage">')
          if (str_detect(subtab, 'CPU')){
           print(ggplot(data = subinfo, aes(x = ReqCPUS, y = CPUEff, colour = str_wrap(Category, 10))) +
              geom_point()+ 
              theme_minimal() + 
          ...
          print(ggplot(data = subinfo, aes(x = ReqMemG, y = MemEff, colour = str_wrap(Category, 10))) +
                  geom_point()+ 
                  theme_minimal() + 
                  theme(panel.background = element_blank(), 
                        legend.title = element_blank(), 
        ...
        print(ggplot(data = subinfo, aes(x = TimelimitRaw, y = TimeEff, colour = str_wrap(Category, 10))) +
              geom_point()+ 
              theme_minimal() + 
              theme(panel.background = element_blank(), 
                    legend.title = element_blank(), 
