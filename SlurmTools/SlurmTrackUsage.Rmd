...
cpu_only[, ':=' (Start = as.POSIXct(Start, format = "%Y-%m-%d %H:%M:%S", origin = "1970-01-01", tz = "UTC"), 
                 End = as.POSIXct(End, format = "%Y-%m-%d %H:%M:%S",origin = "1970-01-01",  tz = "UTC"))]
...
total.dt <- total.dt %>%  
  ...
  mutate(Start = as.POSIXct(Start, tz = 'UTC', origin = "1970-01-01")) %>% 
  mutate(End = ifelse(is.na(End), Start + ElapsedRaw, End)) %>% 
  mutate(End = as.POSIXct(End, tz = 'UTC', origin = "1970-01-01")) %>%  
